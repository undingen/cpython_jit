...
DEFAULT_OUTPUT_JIT_HELPER = os.path.relpath(os.path.join(ROOT, "Python/generated_jit_helper.c.h"))
    ...
    def write_body(self, out: Formatter, dedent: int, cache_adjust: int = 0) -> None:
        ...
        for line in self.block_text:
            if m := re.match(r"(\s*)ERROR_IF\((.+), (\w+)\);\s*(?://.*)?$", line):
                ...
                if symbolic:
                    out.write_raw(
                        f"{space}if ({cond}) {{ STACK_SHRINK({symbolic}); GOTO_{label.upper()}; }}\n"
                    )
                else:
                    out.write_raw(f"{space}if ({cond}) GOTO_{label.upper()};\n")
...
class Analyzer:
    ...
    def write_jit_helper_functions(self) -> None:
        """Write JIT Helper functions to output file."""
        with open(DEFAULT_OUTPUT_JIT_HELPER, "w") as f:
            # Write provenance header
            f.write(f"// This file is generated by {THIS}\n")
            f.write(f"// from {os.path.relpath(self.filename, ROOT)}\n")
            f.write(f"// Do not edit!\n")
            ...
            for thing in self.everything:
                match thing:
                    case parser.InstDef():
                        if thing.kind != "op":
                            # don't generate helper functions for the following opcodes:
                            if thing.name == "EXTENDED_ARG":
                                continue

                            helper_functions.append(thing)

            # write function declarations
            for thing in helper_functions:
                self.out_jit.emit(f"PyObject* JIT_HELPER_{thing.name}(JitHelperState* state, int oparg);")

            # write function definitions
            for thing in helper_functions:
                self.write_jit_helper_instr(self.instrs[thing.name])
    ...
    def write_jit_helper_instr(self, instr: Instruction) -> None:
        name = instr.name

        self.out_jit.emit("")
        with self.out_jit.block(f"PyObject* JIT_HELPER_{name}(JitHelperState* state, int oparg)"):
            instr.write(self.out_jit)
            if not instr.always_exits:
                if instr.check_eval_breaker:
                    self.out_jit.emit("DISPATCH_CHECK_EVAL_BREAKER();")
                else:
                    self.out_jit.emit(f"DISPATCH();")
